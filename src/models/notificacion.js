...
class Notificacion {
    ...
    static async create(usuario_id, mensaje, tipo, estado = 'pendiente') {
        const sql = `
            ...
            VALUES (?, ?, ?, ?)
        `;
        const [result] = await db.pool.promise().execute(sql, [usuario_id, mensaje, tipo, estado]);
    ...
    static async findById(id_notificacion) {
        ...
        const [rows] = await db.pool.promise().execute(sql, [id_notificacion]);
        return rows[0];
    }
    ...
    static async updateEstado(id_notificacion, estado) {
        ...
        const [result] = await db.pool.promise().execute(sql, [estado, id_notificacion]);
    ...
    static async delete(id_notificacion) {
        ...
        const [result] = await db.pool.promise().execute(sql, [id_notificacion]);
        return result.affectedRows > 0;
    }
}
