...
class Actualizacion {
    ...
    static async create(usuario_id, tabla_actualizada, registro_id, campo, valor_anterior, valor_nuevo) {
        const sql = `
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        `;
        const [result] = await db.pool.promise().execute(sql, [usuario_id, tabla_actualizada, registro_id, campo, valor_anterior, valor_nuevo]);
    ...
    static async findById(id_actualizacion) {
        ...
        const [rows] = await db.pool.romise().execute(sql, [id_actualizacion]);
        return rows[0];
    }
    ...
    static async delete(id_actualizacion) {
        ...
        const [result] = await db.pool.promise().execute(sql, [id_actualizacion]);
