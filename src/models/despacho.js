...
class Despacho {
    ...
    static async findById(id_despacho) {
        ...
        const [rows] = await db.pool.promise().execute(sql, [id_despacho]);
        return rows[0];
    }
    ...
    static async update(id_despacho, destino, fecha) {
        ...
        const [result] = await db.pool.promise().execute(sql, [destino, fecha, id_despacho]);
    ...
    static async delete(id_despacho) {
        ...
        const [result] = await db.pool.promise().execute(sql, [id_despacho]);
        return result.affectedRows > 0;
    }
}
