...
class Vehiculo {
  ...
  static async create(placa, marca, modelo, ano, capacidad, tipo_carga, fecha_vencimiento_soat, fecha_vencimiento_tecnomecanica) {
    const sql = `
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const [result] = await db.execute(sql, [placa, marca, modelo, ano, capacidad, tipo_carga, fecha_vencimiento_soat, fecha_vencimiento_tecnomecanica]);
  ...
  static async findById(id_vehiculo) {
    ...
    const [rows] = await db.execute(sql, [id_vehiculo]);
    return rows[0];
  }
  ...
  static async update(id_vehiculo, marca, modelo) {
    ...
    const [result] = await db.execute(sql, [marca, modelo, id_vehiculo]);
  ...
  static async delete(id_vehiculo) {
    ...
    const [result] = await db.execute(sql, [id_vehiculo]);
    return result.affectedRows > 0;
  }
}
